...
public class FeriaEmpresarial {
     ...
     * @return respuesta1
     */
    private String[] zonas = {"Zona norte", "Zona oriente", "Zona occidente", "Zona centro", "Zona sur"};
    ...
    for (Puesto p: puestos) {
        indexZona = 0;
        for (String z: zonas) {
            if (p.estaOcupado() && p.darZona().equals(z)){
            for (Empresa e: empresas) {
                if(p.darNombreEmpresa().equals(e.darNombre())) {
                    contadores[indexZona]+=e.darNumeroPersonasAsistentes();
                    }
             }
            }
            indexZona++;
    ...
    int maxCantidad = 0;
    String maxNombre = "Feria vacía";

    for (int x = 0; x < contadores.length; x++) {
        if (contadores[x] > maxCantidad) {
            maxCantidad = contadores[x];
            maxNombre = zonas[x];
        }
    }
    ...
    String respuesta = "Feria vacía";
    if (maxNombre != "Feria vacía") {
       respuesta = "La zona con mayor número de expositores es la " + maxNombre + " con " +maxCantidad + " expositor(es).";
    }
      return respuesta;
    ...
    public String metodo2() {
        ...
        for (Puesto puesto : puestos) {
                totalPuestos += puesto.darNumMaximoPersonas();
        }

        for (Empresa e: empresas) {
            totalExpositores += e.darNumeroPersonasAsistentes();
        }



        porcentaje = ((double) totalExpositores / totalPuestos) * 100;
        System.out.println(porcentaje);
        String resultado = "El punto de equilibrio no se ha cumplido";
        if (porcentaje >= 60) {
            resultado = "El punto de equilibrio se ha cumplido";
        }
        return resultado;
    }
